...
const struct permonst c_Monsters [CMNUM+2] = {
    {"bat",		'B',  1, 22, 8, 1, 4, 0},
    {"gnome",		'G',  1,  6, 5, 1, 6, 0},
...
{
    const struct permonst* m = monster_by_char (mlet);
    unsigned mi = m - c_Monsters;
    if (!m || mi >= CMNUM)
	return false;
    return _genocided[mi/8] & (1<<(mi%8));
}
...
void make_monster_genocided (char mlet)
{
    ...
    if (!m || mi >= CMNUM)
	return;
    _genocided[mi/8] |= (1<<(mi%8));
}

void savegenocided (int fd)
    { bwrite (fd, ArrayBlock(_genocided)); }
void restgenocided (int fd)
    { mread (fd, ArrayBlock(_genocided)); }
