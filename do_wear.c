...
int doremarm(void)
{
    ...
    otmp = (!uarmh && !uarms && !uarmg) ? uarm : (!uarms && !uarm && !uarmg) ? uarmh : (!uarmh && !uarm && !uarmg) ? uarms : (!uarmh && !uarm && !uarms) ? uarmg : getobj("[", "take off");
    if (!otmp)
	return 0;
...
{
    int delay = -c_Objects[otmp->otyp].oc_delay;
    if (cursed(otmp))
	return 0;
...
int doweararm(void)
{
    ...
    if (otmp == uwep)
	setuwep (NULL);
    delay = -c_Objects[otmp->otyp].oc_delay;
    if (delay) {
	nomul(delay);
...
int dowearring(void)
{
    ...
    if (otmp == uwep)
	setuwep (NULL);
    oldprop = _u.uprops[PROP(otmp->otyp)].p_flgs;
    _u.uprops[PROP(otmp->otyp)].p_flgs |= mask;
    switch (otmp->otyp) {
	case RIN_LEVITATION:
	    if (!oldprop)
		float_up();
...
void ringoff(struct obj *obj)
{
    ...
    if (!(_u.uprops[PROP(obj->otyp)].p_flgs & mask))
	impossible("Strange... I didnt know you had that ring.");
    _u.uprops[PROP(obj->otyp)].p_flgs &= ~mask;
    switch (obj->otyp) {
	case RIN_FIRE_RESISTANCE:
	    // Bad luck if the player is in hell... --jgm
	    if (!Fire_resistance && _u.dlevel >= 30) {
...
void glibr(void)
{
	...
	if (uleft || uright) {
	    ...
	    pline("Your %s off your fingers.", (uleft && uright) ? "rings slip" : "ring slips");
	    ++xfl;
	    if ((otmp = uleft)) {
		ringoff(uleft);
		dropx(otmp);
...
void corrode_armor(void)
{
    ...
    if (otmph) {
	if (otmph->rustfree || otmph->otyp == ELVEN_CLOAK || otmph->otyp == LEATHER_ARMOR || otmph->otyp == STUDDED_LEATHER_ARMOR) {
